...
def main():
    ...
    deep = Deep(x_train.shape[0], [12, 1])
    iterations = 500000
    A, cost = deep.train(x_train, y_train, iterations=iterations, alpha=0.15)
    deep.save('census7_12_1_FULL_{}'.format(iterations))
    ...
    y_mean = np.mean(y_test_pred)
    y_test_pred = np.heaviside(y_test_pred - y_mean, 1).astype(int)
    result = pd.DataFrame(
        np.stack(
            (
                np.array(test['Id']),
                y_test_pred.reshape(y_test_pred.shape[1])
            ),
            axis=1
        ),
        columns=['Id', 'Predicted']
    )
    result.to_csv('census7_12_1_FULL_{}.csv'.format(iterations), index=False)
